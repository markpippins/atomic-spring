package com.angrysurfer.spring.atomic.shrapnel.component.writer;
...
import com.angrysurfer.spring.atomic.shrapnel.field.IValueFormatter;
import com.angrysurfer.spring.atomic.shrapnel.filter.DataFilters;
import com.angrysurfer.spring.atomic.shrapnel.filter.IDataFilters;
import com.angrysurfer.spring.atomic.shrapnel.component.writer.style.provider.CombinedStyleProvider;
import com.angrysurfer.spring.atomic.shrapnel.component.writer.style.provider.StyleProvider;
...
public class PdfDataWriter extends DataWriter {
    ...
    protected Cell createCell(Object item, IField field, int row) {
        ...
        if (Objects.nonNull(field))
            if (PADDING_COLUMNS.contains(field))
                cell.add(new Paragraph(field.getLabel()));
            else cell.add(new Paragraph(getValue(item, field)));
    ...
    protected Cell createHeaderCell(IField field) {
        ...
        return cell.add(new Paragraph(field.getLabel()));
    }

    public Table createTable() {
                Table table = new Table(UnitValue.createPercentArray(getTableWidths()));
